...
const INSERT_APP_CFG = `INSERT INTO
    hc_console_system_cluster_apps_config (cluster_code, app, config, version, user, gmt_create)
  VALUES(?, ?, ?, ?, ?, ?);`;
...
AppConfig.save = (appCfg, callback) => {
  ...
  db.query(
    INSERT_APP_CFG,
    [appCfg.clusterCode, appCfg.app, JSON.stringify(appCfg.config), d.getTime(), appCfg.user, d],
    function (err) {
      if (err) {
...
const GET_APP_CFG = `
  ...
  WHERE cluster_code = ? and app = ? order by version desc limit 1;
`;
AppConfig.getAppConfig = (appCfg, callback) => {
  let d = new Date();
  db.query(
    GET_APP_CFG,
...
const GET_CLUSTER_APP_CFGS = `
  ...
  FROM hc_console_system_cluster_apps_config
  WHERE cluster_code = ? group by cluster_code, app;
`;
